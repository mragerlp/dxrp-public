...
/// <summary>
/// Editor-only equip command for the local M1911 mechanical graybox.
/// The command deliberately fails closed until the current game-mode roster
/// contains a Portal row for this exact world-prefab path.
/// </summary>
public static class M1911DevGive
{
	private const string Command = "lp_give_m1911";
	private const string WorldPrefabPath = "addons/lifepunch/lpweapons/m1911/equipment/w_m1911/w_m1911.prefab";
	private const string ViewPrefabPath = "addons/lifepunch/lpweapons/m1911/equipment/vm_m1911/vm_m1911.prefab";

	[ConCmd( Command )]
	public static void GiveM1911()
	{
		...
		GiveM1911To( Player.Local );
	}

	public static bool GiveM1911To( Player player, string logPrefix = Command )
	{
		...
		{
			Log.Error( $"{logPrefix}: roster miss or viewmodel mismatch for the exact M1911 Portal row; " +
		...
		Log.Info( $"{logPrefix}: created M1911 on '{player.SteamName}' active={active} " +
	...
	private static void RemoveExisting( Player player, Guid equipmentId, Equipment except )
	{
		foreach ( var weapon in player.Equipment.ToList() )
		{
			...
			if ( weapon.EquipmentId == equipmentId
				|| string.Equals( weapon.GameObject.Name, "w_m1911", StringComparison.OrdinalIgnoreCase ) )
			{
