...
/// <summary>
/// Editor-only equip command for the M870 private Portal candidate.
/// The command deliberately fails closed until the current game-mode roster
/// contains the exact M870 world-prefab row.
/// </summary>
public static class M870DevGive
{
	private const string Command = "lp_give_m870";
	private const string WorldPrefabPath = "addons/lifepunch/lpweapons/m870/equipment/w_m870/w_m870.prefab";
	private const string ViewPrefabPath = "addons/lifepunch/lpweapons/m870/equipment/vm_m870/vm_m870.prefab";

	[ConCmd( Command )]
	public static void GiveM870()
	{
		...
		GiveM870To( Player.Local );
	}

	public static bool GiveM870To( Player player, string logPrefix = Command )
	{
		...
		{
			Log.Error( $"{logPrefix}: roster miss or viewmodel mismatch for the exact M870 Portal row; " +
		...
		Log.Info( $"{logPrefix}: created M870 on '{player.SteamName}' active={active} " +
	...
	private static void RemoveExisting( Player player, Guid equipmentId, Equipment except )
	{
		foreach ( var weapon in player.Equipment.ToList() )
		{
			...
			if ( weapon.EquipmentId == equipmentId
				|| string.Equals( weapon.GameObject.Name, "w_m870", StringComparison.OrdinalIgnoreCase ) )
			{
