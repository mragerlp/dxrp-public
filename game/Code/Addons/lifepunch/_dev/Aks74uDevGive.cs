...
/// <summary>
/// Editor-only equip command for the AKS-74U private Portal candidate.
/// The command deliberately fails closed until the current game-mode roster
/// contains the real Portal row for the AKS-74U world prefab.
/// </summary>
public static class Aks74uDevGive
{
	private const string Command = "lp_give_aks74u";
	private const string WorldPrefabPath = "addons/lifepunch/lpweapons/aks74u/equipment/w_aks74u/w_aks74u.prefab";
	private const string ViewPrefabPath = "addons/lifepunch/lpweapons/aks74u/equipment/vm_aks74u/vm_aks74u.prefab";

	[ConCmd( Command )]
	public static void GiveAks74u()
	{
		...
		GiveAks74uTo( Player.Local );
	}

	public static bool GiveAks74uTo( Player player, string logPrefix = Command )
	{
		...
		{
			Log.Error( $"{logPrefix}: roster miss or viewmodel mismatch for the AKS-74U Portal row; " +
		...
		Log.Info( $"{logPrefix}: created AKS-74U on '{player.SteamName}' active={active} " +
	...
	private static void RemoveExisting( Player player, Guid equipmentId, Equipment except )
	{
		foreach ( var weapon in player.Equipment.ToList() )
		{
			...
			if ( weapon.EquipmentId == equipmentId
				|| string.Equals( weapon.GameObject.Name, "w_aks74u", StringComparison.OrdinalIgnoreCase ) )
			{
